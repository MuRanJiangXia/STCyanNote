...
+ (id)instanceWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)bundleOrNil owner:(id)owner {
    ...
    if (nib) {
        ...
        NSAssert([view isKindOfClass:self], @"First object in nib '%@' was '%@'. Expected '%@'",
                 nibName, view, self);
        return view;
    }
...
- (UIView *)viewOfClass:(Class)viewClass {
    return
        [self viewMatchingPredicate:[NSPredicate
...
- (UIView *)firstSuperviewOfClass:(Class)viewClass {
    return
        [self firstSuperviewMatchingPredicate:
                  [NSPredicate predicateWithBlock:^BOOL(UIView *superview, __unused id bindings) {
...
- (BOOL)viewOrAnySuperviewIsKindOfClass:(Class)viewClass {
    return
        [self viewOrAnySuperviewMatchesPredicate:
                  [NSPredicate predicateWithBlock:^BOOL(UIView *superview, __unused id bindings) {
                      return [superview isKindOfClass:viewClass];
                  }]];
}
...
- (UIView *)firstResponder {
    return [self viewMatchingPredicate:[NSPredicate predicateWithBlock:^BOOL(id evaluatedObject,
                                                                             __unused id bindings) {
...
/**
 *  柏江舟 2014年06月22日00:34:05
 *  用来设置在 parent view中的左右间距
 *
 *  @return <#return value description#>
 */
- (CGFloat)rightPadding {
    return self.superview.width - (self.origin.x + self.size.width);
}

- (void)setRightPadding:(CGFloat)rightPadding {
    CGRect frame = self.frame;
    frame.origin.x = self.superview.width - rightPadding - self.size.width;
    self.frame = frame;
}
